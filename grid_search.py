...
def main():
    ...
    # Purge the original massive checkpoint pools from System RAM completely
    del global_pool
    del local_pool
    del semantic_pool
    gc.collect()

    # Prepare combinations to distribute using our safe lists
    combinations = []
    for g_key in global_base_keys:
        for l_key in local_base_keys:
            for s_key in semantic_base_keys:
                for k in TOP_K_SEARCH:
                    for mode in MODES:
